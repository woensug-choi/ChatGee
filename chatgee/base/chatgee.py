...
class ChatGeeOBJ:
    ...
    def __init__(self, ChatGee_Config):
        ...
        self.system_prompt_tokens_estimate = len(self.encoding.encode(self.ChatGee_Config['SETTINGS']['SYSTEM_PROMPT'])) + 350 # 320 by ChatGee Template
        self.max_user_prompt_history_tokens = int(4096/2) - self.system_prompt_tokens_estimate
        # Set OpenAI Module Configuratoin
        self.OpenAI = ChatGee_OpenAI(api_key = self.ChatGee_Config['OPEN_AI']['API_KEY'],
    ...
    def prompt_received(self, content):
        ...
        user_data = list(self.DB.get_user_data_by_room(userid)[0])
        if previous_friendship == False and isFriend == True:
        ...
        # Check membership
        if user_data[2] == False and user_data[4] > self.ChatGee_Config['SETTINGS']['NO_FRIEND_USE_LIMIT']:
            ...
            response = ChatGee_KakaoTalk.insert_button_text(response, '이전 말 다시 적기 ✍🏻', chat_history[0][3])
            run_flag = False
        ...
        if run_flag:
            ...
            if user_data[4] % self.ChatGee_Config['SETTINGS']['ADVERTISEMENT_FREQUENCY'] == 0 and content['action']['detailParams']['prompt']["value"] != '생각 다 했니???!':
    ...
    def prompt(self, request_queue, response_queue):
        ...
        # create the thread
        chat_gpt_respond = threading.Thread(target=self.respond, args=(child_queue, content, userid))
    ...
    def chatgpt_wrapper(self, prompt, userid, repeat):
        ...
        # Normal function if not repeating
            ...
            prompt_tot.insert(0, {"role": "system", "content": "You are a kind and helpful ChatGee identifies as 'ChatGee'(abbreviation of'ChatGPT') who is 1:1 conversation with the User. ChatGee will reply as short as possible and add emojis effectively." \
